...
- (void)_scrollRangeToVisible:(YYTextRange *)range {
        ...
        if (!CGRectIsNull(inter) && inter.size.height > 1 && inter.size.width > extend) { // self is covered by keyboard
            if (CGRectGetMinY(inter) > CGRectGetMinY(bounds)) { // keyboard below self.top
                ...
                if (originalContentInset.bottom < inter.size.height + extend) {
                    ...
                    [UIView animateWithDuration:0.0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction | curve animations:^{
    ...
    if (!insetModified) {
        ...
        [UIView animateWithDuration:0.0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationCurveEaseOut animations:^{
